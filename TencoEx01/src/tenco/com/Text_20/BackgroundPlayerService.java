...
public class BackgroundPlayerService implements Runnable {
	...
	public BackgroundPlayerService(Player player) {
		...
		this.bubbles = player.getBubbles();//플레이어에 겟버블즈를 들고올거다
		try {
		...
		while (true) {
			...
			for(int i = 0; i < bubbles.size(); i ++) {
				...
				if(targetBubble.getState() == 1) {
					if((Math.abs(player.getX() - targetBubble.getX()) < 10)//x좌표값이 10보다 작을때
						&& (Math.abs(player.getY() - targetBubble.getY()) > 0) 
						&& (Math.abs(player.getY() - targetBubble.getY()) < 50)){// y좌표값이 0보다 크고 50보다작을때        
