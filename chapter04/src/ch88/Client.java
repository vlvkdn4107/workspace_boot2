...
public class Client extends JFrame implements ActionListener {
	...
	private void init() {
		...
		panel_1.add(hostIP_tf);//로그인 패널 안에 Host_IP에 입력할수있는 텍스트필드 추가
		...
		panel_1.add(port_tf);//로그인 패널 안에 Server_Port에 입력할수있는 텍스트필드 추가
		...
		panel_1.add(userID_tf);//로그인 패널 안에 User_ID에 입력할수있는 텍스트필드 추가
		...
		panel_1.add(connect_btn);//로그인 패널 안에 connect에 입력할수있는 텍스트필드 추가
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == connect_btn) {
			if(hostIP_tf.getText().length() ==0) {
				...
				port_tf.setText("포트번호를 입력하세요");//port_tf텍스트 필드에 아무것도 없으면 나오는 메세지
				port_tf.requestFocus();//다음 입력을 편하게 받기 위해서 TextField에 포커스 요청
			} else if(userID_tf.getText().length() == 0) {
				userID_tf.setText("id 를 입력하세요");//userID_tf텍스트 필드에 아무것도 없으면 나오는 메세지
				userID_tf.requestFocus();//다음 입력을 편하게 받기 위해서 TextField에 포커스 요청
			} else {
				ip = hostIP_tf.getText();
				try{
				port = Integer.parseInt(port_tf.getText().trim());
				}catch (Exception e2) {
					port_tf.setText("잘못 입력하였습니다.");//port_tf에 잘못된 포트번호를 입력해서 입장할려고하면 뜨는 메세지
				}
				user_id = userID_tf.getText().trim();
				// 서버연결하기
				connectServer();
			...
			if(chatting_tf.getText().length() == 0 ){
				System.out.println("이게 0값으로 들어가나?");// java.util.NoSuchElementException 오류뜸
				sendmessage("Chatting/" + my_roomName + "/"
						+ chatting_tf.getText()+"   ");
			}else {
				sendmessage("Chatting/" + my_roomName + "/"
